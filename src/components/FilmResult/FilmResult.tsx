...
const FilmResult = ({ film }: FilmResultProps) => {
  return (
    <Link key={film.id} href={`/film/${film.id}`} passHref>
      <a className="bg-black-bright text-white flex flex-col md:flex-row cursor-pointer rounded-lg shadow-slate-700 shadow-[0_0_2px] hover:shadow-[0_0_5px] duration-200">
        <img
        ...
        <div className="ml-2 py-3 flex-1">
          ...
          <p className="text-sm md:text-base" data-testid="overview">
            {film.overview
              ? film.overview
              : "Este filme não contem descrição :("}
          </p>
        </div>
      </a>
    </Link>
